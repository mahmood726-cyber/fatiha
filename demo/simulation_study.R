  ...
  for (i in 1:n_sim) {
    ...
    if (!is.null(fit_fe)) {
      ...
      results[[idx]] <- data.frame(
        Method = "FE",
        Estimate = as.numeric(fit_fe$beta),
        SE = fit_fe$se,
        Lower = fit_fe$ci.lb,
        Upper = fit_fe$ci.ub
      )
    ...
    if (!is.null(fit_re)) {
      ...
      results[[idx]] <- data.frame(
        Method = "REML",
        Estimate = as.numeric(fit_re$beta),
        SE = fit_re$se,
        Lower = fit_re$ci.lb,
        Upper = fit_re$ci.ub
      )
    ...
    if (!is.null(fit_egger)) {
      ...
      est <- coef(fit_egger)[1]
      se <- sum_e$coefficients[1, 2]
      idx <- idx + 1
      results[[idx]] <- data.frame(
        Method = "Egger",
...
write.csv(all_results, file.path(DATA_DIR, "simulation_results_extended.csv"), row.names = FALSE)
