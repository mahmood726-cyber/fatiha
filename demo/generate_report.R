...
run_full_sim <- function(k, tau2, bias, scenario_name, n_sim = 200) {
  ...
  for (i in 1:n_sim) {
    ...
    if (!is.null(f)) {
      results[[length(results) + 1]] <- data.frame(Method = "FE", Est = as.numeric(f$beta), L = f$ci.lb, U = f$ci.ub)
    }

    r <- tryCatch(rma(yi, vi, method = "REML"), error = function(e) NULL)
    if (!is.null(r)) {
      results[[length(results) + 1]] <- data.frame(Method = "REML", Est = as.numeric(r$beta), L = r$ci.lb, U = r$ci.ub)
    }

    e_fit <- tryCatch(lm(yi ~ sei, weights = 1 / vi), error = function(e) NULL)
    if (!is.null(e_fit)) {
      e_est <- coef(e_fit)[1]
      e_se <- summary(e_fit)$coefficients[1, 2]
      results[[length(results) + 1]] <- data.frame(
        Method = "Egger",
        Est = as.numeric(e_est),
        L = e_est - qnorm(0.975) * e_se,
        U = e_est + qnorm(0.975) * e_se
      )
    }

    fa <- tryCatch(synthesis_meta(yi, vi), error = function(e) NULL)
    if (!is.null(fa)) {
      results[[length(results) + 1]] <- data.frame(Method = "SYNTHESIS", Est = fa$estimate, L = fa$ci_lb, U = fa$ci_ub)
    }
  }

  df <- do.call(rbind, results)
  do.call(rbind, lapply(split(df, df$Method), function(x) {
    data.frame(
...
figure_path <- file.path(DEMO_DIR, "Performance_Summary.png")
...
barplot(
  dat_bias$Bias,
  names.arg = dat_bias$Method,
  col = c("gold", "darkred", "darkblue", "darkgreen"),
  main = "Bias in Presence of Publication Bias",
  ylab = "Estimate Deviation from Truth (0.5)"
)
abline(h = 0, lty = 2)
...
barplot(
  dat_cov$Coverage,
  names.arg = dat_cov$Method,
  col = c("gold", "darkred", "darkblue", "darkgreen"),
  main = "CI Coverage (Target: 0.95)",
...
barplot(
  dat_mse$MSE,
  names.arg = dat_mse$Method,
  col = c("gold", "darkred", "darkblue", "darkgreen"),
  main = "MSE in Small Samples (k=12)",
  ylab = "Mean Squared Error (Lower is better)"
)

dat_ideal <- all_results[all_results$Scenario == "Ideal", ]
barplot(
  dat_ideal$Bias,
  names.arg = dat_ideal$Method,
  col = c("gold", "darkred", "darkblue", "darkgreen"),
  main = "Bias in Ideal Conditions",
  ylab = "Bias"
)

dev.off()

write.csv(all_results, file.path(DATA_DIR, "simulation_results.csv"), row.names = FALSE)
cat(sprintf("%s created successfully.\n", figure_path))
