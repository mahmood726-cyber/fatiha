...
#'
#' Loops over all available Cochrane reviews, extracts binary outcome analyses,
...
files <- list.files(PAIRWISE_DIR, pattern = "\\.rda$", full.names = TRUE)
...
if (length(results) > 0) {
  ...
  cat(sprintf(
    "SYNTHESIS CI covers REML estimate: %d/%d (%.1f%%)\n",
    sum(benchmark$SYN_covers_REML), nrow(benchmark), 100 * mean(benchmark$SYN_covers_REML)
  ))
  cat(sprintf(
    "Significance agreement: %d/%d (%.1f%%)\n",
    sum(benchmark$Sig_agree), nrow(benchmark), 100 * mean(benchmark$Sig_agree)
  ))
  cat(sprintf(
    "Median CI width — REML: %.4f | SYNTHESIS: %.4f\n",
  ...
  for (bin in levels(k_bins)) {
    ...
    cat(sprintf(
      "  %s (n=%d): median diff=%.4f, sig agree=%.1f%%\n",
      bin, nrow(sub), median(sub$Abs_diff), 100 * mean(sub$Sig_agree)
    ))
  }

  write.csv(benchmark, file.path(DATA_DIR, "pairwise70_benchmark.csv"), row.names = FALSE)
